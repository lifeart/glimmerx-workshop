...
const activeClass = positionalHelper((isActive: boolean) => {
...
export default class UsersRoute extends Component<{
  ...
  get model() {
    return [...this.state.users, ...this.args.model].map((login) => {
      return {
        login,
        isActive: this.router.activeRoute?.page.params.login === login,
        canDelete: !this.args.model.includes(login)
      }
    })
  }
        ...
        {{#each this.model as |user|}}
          <li class="text-left hover:text-black underline bg-blue-100 rounded-md mb-2 mt-2 p-1">
            <a href="/users/{{user.login}}" 
              class={{activeClass user.isActive}}
            >{{user.login}}</a>
            {{#if user.canDelete}}
              <button type="button"
                {{on 'click' (fn this.remove user.login)}}
              >[x]</button>
            {{/if}}
          </li>
        {{/each}}
